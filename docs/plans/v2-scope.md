...
|---|---|---|
| 4 features trigger FLAG_SECURE re-enable; first trigger to ship pays the integration cost | V2.1, V2.2, V2.8, V2.9 | Whichever V2 feature ships first writes a small helper (`enableFlagSecureOnActivity()`) that the other three reuse. Document the helper in `ADR-AND-R` amendment when it lands. |
