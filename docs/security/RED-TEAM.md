...
- ✓ AUDIO FOCUS FIXED (Sprint 20): `EpubReaderViewModel.startTts()` now requests `AudioManager.AUDIOFOCUS_GAIN_TRANSIENT` before `nav.play()`. `cleanUpTts()` calls `abandonAudioFocusRequest()`. The focus-change listener calls `pauseTts()` on `AUDIOFOCUS_LOSS` / `AUDIOFOCUS_LOSS_TRANSIENT` so phone calls and navigation prompts correctly interrupt TTS. `GAIN_TRANSIENT` (not `MAY_DUCK`) is used because TTS is spoken word — competing audio must pause, not merely duck; `MAY_DUCK` would deliver `AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK` to music apps, which our listener ignores, causing both streams to play simultaneously. `requestAudioFocus` return value is checked: if `AUDIOFOCUS_REQUEST_FAILED` (e.g., active phone call), TTS is silently skipped. Prior to this fix, `TextToSpeech.speak()` played on top of other audio — Readium's `TtsNavigator` explicitly delegates audio-focus management to the app.
- ✅ REGRESSION TESTS:
